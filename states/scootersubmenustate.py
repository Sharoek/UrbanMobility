...
from encryption.validators import verify_number_input, validate_and_normalize_coord, check_mileage, validate_last_maintanence, validate_brand, validate_serial_number, validate_model

class scooterSubMenuState(AppState):
    #A submenu can be differntiated based on the user role. 
    def __init__(self, context, scooter):
    ...
    def run(self):
        ...
        try:
            if self.context.role == "system_admin" or self.context.role == "super_admin":
                self.show_admin_sub_menu(self.scooter)
            if self.context.role == "service_engineer":
                self.show_sub_menu(self.scooter)
