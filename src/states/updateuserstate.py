    ...
    def updateUserMenu(self):
        while True:
            ...
            try:
                ...
                self.log_manager.log(self.context.username, "update user", f"Failed to update user: {e}", True)
                break
    ...
    def update_user_by_id(self, user_id):
        try:
            ...
            if choice == 1:
                ...
                self.log_manager.log(self.context.username, "update user", f"Updated username for user ID {user_id} to {username}")
            ...
            #     self.context.admin_repo.update_user(user_id, "password", hashed_pw)
                ...
                self.log_manager.log(self.context.username, "update user", f"Updated first name for user ID {user_id} to {first_name}")
                ...
                self.log_manager.log(self.context.username, "update user", f"Updated last name for user ID {user_id} to {last_name}")

            print("✅ User updated successfully.")

        except Exception as e:
            print(f"An error occurred: {e}")
            self.log_manager.log(self.context.username, "update user", f"Failed to update user ID {user_id}: {e}", True)
